...
def evaluate(context):
    ...
    for candidate in context["observations"]:
        ...
        if candidate["venueId"] != obs["venueId"] and candidate["outcomeLabel"].lower() == obs["outcomeLabel"].lower() and candidate_price is not None:
  ...
  },
  {
    ...
    decision = "context-only" if obs.get("routeState") != "paper-executable" and passes else ("accept" if passes else "reject")
